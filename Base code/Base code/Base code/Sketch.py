...
class Sketch(CanvasBase):
    ...
    # internal storage of layer data
    layerList = {}
    layerIndex = 0
    layer = None
    ...
    def __init__(self, parent):
        ...
        file  = open("curveCoords.txt","w")
        file.close()

        # set up the first layer
        self.layerList[self.layerIndex] = Layer(self.layerIndex)
        self.layer = self.layerList[self.layerIndex]
    ...
    def Interrupt_MouseL(self, x, y):
        ...
        if len(self.points_l) % 2 == 1:
            ...
            # save the line in the current layer
            self.layer.addLine(self.points_l[-1], self.points_l[-2])
    ...
    def Interrupt_MouseR(self, x, y):
        ...
        if len(self.points_r) % 4 == 1:
            ...
            # save the curve in the current layer
            self.layer.addCurve(self.points_r[-4],self.points_r[-3],self.points_r[-2],self.points_r[-1])
    ...
    def switchLayers(self, interval):
        self.layerIndex += interval

        if not (self.layerIndex in self.layerList):
            self.layerList[self.layerIndex] = Layer(self.layerIndex)

        self.layer = self.layerList[self.layerIndex]

    def drawLayer(self):
        for object in self.layer.data:
            if object.type == LayerObject.LINE:
                self.drawLine(self.buff, object.data[0], object.data[1])
            elif object.type == LayerObject.CURVE:
                self.drawBezierCurve(self.buff, object.data[0], object.data[1], object.data[2], object.data[3])
...
if __name__ == "__main__":
    def main():
        ...
        frame = wx.Frame(None, size=(800, 800), title="CS581 UI Test", style=wx.DEFAULT_FRAME_STYLE | wx.FULL_REPAINT_ON_RESIZE)
